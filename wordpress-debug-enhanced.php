<?php
/**
 * ENHANCED DEBUG VERSION - Better messaging and multiple send attempts
 * Replace your current WordPress code with this version for better debugging
 */

// Enhanced shortcode with better debugging
function meal_planner_button_shortcode($atts) {
                ...
                if (isset($ingredient_group['ingredients']) && is_array($ingredient_group['ingredients'])) {
                    foreach ($ingredient_group['ingredients'] as $ingredient) {
                        $recipe_data['ingredients'][] = array(
                            ...
                            'amount' => isset($ingredient['amount']) ? $ingredient['amount'] : '1', // Keep both
    ...
    return '<div style="margin: 20px 0; text-align: center; background: linear-gradient(135deg, #fef7ed 0%, #fed7aa 100%); border: 2px solid #fb923c; border-radius: 16px; padding: 20px;">
        ...
        ' . (current_user_can('administrator') ? '<div style="background: green; color: white; padding: 5px; margin-bottom: 10px; border-radius: 5px; font-size: 12px;">
            ✅ Enhanced Debug Button: ' . esc_html($recipe->name()) . ' (ID: ' . $recipe_id . ') → Development URL
        </div>' : '') . '
        
        <button onclick="debugMealPlannerImport(\'' . $recipe_json_safe . '\')" style="
...
// Enhanced JavaScript with extensive debugging
function add_debug_meal_planner_script() {
    if (is_single() && has_shortcode(get_post()->post_content, 'meal_planner_button')) {
        ?>
        <script>
        function debugMealPlannerImport(recipeJsonString) {
            console.log('🍽️ DEBUG IMPORT starting...');
            console.log('📊 Raw JSON string length:', recipeJsonString.length);
            
            try {
                const recipe = JSON.parse(recipeJsonString);
                console.log('✅ Recipe data parsed successfully:', recipe);
                ...
                showDebugFeedback('Opening meal planner...', 'info');
                
                // Multiple attempts to send recipe with extensive logging
                const sendAttempts = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000];
                let attemptCount = 0;
                ...
                sendAttempts.forEach((delay, index) => {
                    setTimeout(() => {
                        ...
                        attemptCount++;
                        console.log(`📤 Attempt ${attemptCount}: Sending recipe data...`);
                        console.log(`🎯 Target domain: ${developmentUrl}`);
                        console.log(`📊 Sending recipe:`, recipe.title || recipe.name);
                        
                        try {
                            popup.postMessage({
                                type: 'IMPORT_RECIPE',
                                recipe: recipe,
                                source: 'wordpress-debug',
                                timestamp: Date.now(),
                                attempt: attemptCount,
                                debug: true
                            }, developmentUrl);
                            
                            console.log(`✅ Attempt ${attemptCount} sent successfully`);
                            
                            if (attemptCount === 1) {
                                showDebugFeedback('Recipe data sent! Check meal planner...', 'success');
                            }
                        } catch (error) {
                            console.warn(`⚠️ Attempt ${attemptCount} failed:`, error);
                            if (attemptCount === sendAttempts.length) {
                                showDebugFeedback('All attempts completed. Recipe should be available when app loads.', 'info');
                            }
                        }
                    }, delay);
                });
                ...
                setTimeout(() => {
                    try {
                        console.log('🚀 Immediate send attempt...');
                        popup.postMessage({
                            type: 'IMPORT_RECIPE',
                            recipe: recipe,
                            source: 'wordpress-debug',
                        ...
                        console.log('✅ Immediate send completed');
                    } catch (error) {
                        console.log('⚠️ Immediate send failed, will retry...', error);
                    }
                }, 500);
                
            } catch (error) {
                console.error('❌ Debug import error:', error);
                showDebugFeedback('Error importing recipe: ' + error.message, 'error');
            }
        }
        ...
        function showDebugFeedback(message, type) {
            ...
            const existing = document.getElementById('debug-feedback');
            if (existing) existing.remove();
            
            // Create feedback element
            const feedback = document.createElement('div');
            feedback.id = 'debug-feedback';
...
add_action('wp_footer', 'add_debug_meal_planner_script');
...
function show_debug_info() {
    if (is_single() && current_user_can('administrator')) {
        ...
        echo '<div style="background: blue; color: white; padding: 10px; position: fixed; top: 10px; right: 10px; z-index: 9999; border-radius: 8px;">
            🔧 DEBUG Integration Active!<br>
            Found ' . $recipe_count . ' recipe(s)<br>
            <small>→ Multiple send attempts with extensive logging</small>
        </div>';
    }
}
