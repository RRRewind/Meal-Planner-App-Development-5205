<?php
/**
 * Complete Meal Planner Integration for WP Recipe Maker
 * Enhanced version with Gutenberg block detection
 * Multi-domain support for development and production
 * 
 * INSTRUCTIONS:
 * 1. Replace your test code with this complete version
 * 2. Upload wordpress-integration.js to your theme folder
 * 3. Test on a recipe post
 */
    ...
    if (is_single()) {
        ...
        if (!empty($recipe_ids)) {
            wp_enqueue_script(
                ...
                array(),
                '1.3', // Version number for cache busting
                true
            );
        }
    }
...
function add_meal_planner_script_to_footer() {
    if (is_single()) {
        ...
        if (!empty($recipe_ids)) {
            ?>
            <script src="<?php echo get_template_directory_uri(); ?>/wordpress-integration.js?v=1.3"></script>
            <?php
        }
    }
}
