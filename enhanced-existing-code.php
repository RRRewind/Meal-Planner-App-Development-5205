...
function meal_planner_button_shortcode($atts) {
    ...
    // NEW: Add chill time if available
    if (method_exists($recipe, 'total_time')) {
        $recipe_data['chillTime'] = max(0, $recipe->total_time() - $recipe_data['prep_time'] - ($recipe_data['cook_time'] ?? 0));
    }
                ...
                if (isset($ingredient_group['ingredients']) && is_array($ingredient_group['ingredients'])) {
                    foreach ($ingredient_group['ingredients'] as $ingredient) {
                        $recipe_data['ingredients'][] = array(
                            ...
                            'category' => 'Other', // NEW: Add category for shopping list organization
